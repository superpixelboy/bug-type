// o_bug_card_collection Draw_64 Event - EXACT MATCH TO ORIGINAL
...
// MATCH ORIGINAL: Use same scaling as o_bug_card
var card_scale_x = 1.0;  // Match original card scaling
var card_scale_y = 1.0;
var flip_scale = 1.0;    // No flip animation for collection view

// MATCH ORIGINAL: Calculate frame dimensions exactly the same way
...
// MATCH ORIGINAL: Pixel-perfect positioning
...
// ===== CARD BACKGROUND (exactly like original) =====
draw_sprite_ext(
    s_card_template, 1,  // frame 1 = front
    cx_gui, cy_gui,
    scale_x_template, scale_y_template,
...
if (content_ready && content_fade_alpha > 0) {
    ...
    // ===== COIN (exactly like original - mirrored) =====
    if (gem_pop_scale > 0) {
        ...
        // MATCH ORIGINAL: Coin number with exact styling
        ...
        // MATCH ORIGINAL: 8-direction outline
    ...
    // Build text surface for content (exactly like original)
    if (bug_pop_scale > 0) {
        ...
        if (surface_exists(text_surf)) {
            ...
            // ===== BUG SPRITE (exactly like original) =====
            ...
            // ===== BUG NAME (exactly like original) =====
            draw_set_font(fnt_card_title_2x);
            var cream = make_color_rgb(245,235,215);
            ...
            // MATCH ORIGINAL: Semi-transparent name shadow
            ...
            // ===== FLAVOR TEXT (exactly like original) =====
            ...
            // Draw the surface to screen with exact original scaling
            var surface_x = cx_gui - (surf_w * 0.5);
            var surface_y = cy_gui - (surf_h * 0.5);
            draw_surface(text_surf, surface_x, surface_y);
...
if (card_state == "displayed") {
    ...
    var instruction_y = cy_gui + frame_h_gui * 0.45;
    
    // Instruction shadow
    draw_set_alpha(0.7);
    draw_set_color(c_black);
    draw_text(cx_gui + 3, instruction_y + 3, "Click to return");
    draw_set_alpha(1);
    
    // Main instruction text
    draw_set_color(c_white);
    draw_text(cx_gui, instruction_y, "Click to return");
}
