...
// Layer 3: Title and menu items using your beautiful s_title_title sprite
...
// Only show menu after title is fully visible
if (title_fully_visible) {
    ...
    draw_set_alpha(1); // Full opacity for menu
    draw_set_halign(fa_center);
    draw_set_valign(fa_middle);
    ...
    for (var i = 0; i < array_length(menu_items); i++) {
        ...
        if (!item.enabled) {
            ...
            // Selected item with outline (matching your UI style)
            var outline_color = make_color_rgb(75, 50, 130);
            var outline_offset = 1 * gui_scale;
            
            // Draw outline in 8 directions
            draw_set_color(outline_color);
            for (var ox = -outline_offset; ox <= outline_offset; ox += outline_offset) {
                for (var oy = -outline_offset; oy <= outline_offset; oy += outline_offset) {
                    if (ox != 0 || oy != 0) {
                        draw_text_transformed(title_x + ox, item_y + oy, item.text, gui_scale, gui_scale, 0);
                    }
                }
            }
